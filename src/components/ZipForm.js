...
class ZipForm extends Component {
      ...
      <div className="zip-form">
        <form id="zipForm" onSubmit={this.submitZipCode}>
            <div className="flex-parent">
              ...
              <input className="form-control" type="input" id="zipcode" name="zipcode"  value={this.state.zipcode} onChange={this.inputChanged} required  />
  ...
  inputChanged(e) {
    const {value} = e.target; //get the value from the event
    this.setState( {zipcode: value} ); //set the state of zipcode to the value
  }

  submitZipCode (e) {
    e.preventDefault(); // don’t submit the form
    const { zipcode } = this.state; // get the zipcode from the state
    const { onSubmit } = this.props; // the onSubmit method from the App 
